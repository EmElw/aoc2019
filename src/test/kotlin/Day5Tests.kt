...
class Day5Tests {
    @Test
    fun `Example 1`() {
        val ic1 = intCodeExec(listOf(3, 0, 4, 0, 99).map(Int::toBigInteger), input = listOf(1337.toBigInteger()))
        val ic2 = intCodeExec(listOf(1002, 4, 3, 4, 33).map(Int::toBigInteger))
        Assertions.assertEquals(1337.toBigInteger(), ic1.output.first())
        Assertions.assertEquals(listOf(1002, 4, 3, 4, 99).map(Int::toBigInteger), ic2.code)
    }
    ...
    fun `Less than tester`() = listOf(
        ...
        DynamicTest.dynamicTest("Equality tester: input $input, expected $expected") {
            Assertions.assertEquals(
                ...
                intCodeExec(
                    code = input.first.map(Int::toBigInteger),
                    input = listOf(input.second.toBigInteger())
                ).output.first()
            )
        }
    }
